...
class VillageEmissionReportPage extends StatefulWidget {
  const VillageEmissionReportPage({super.key});

  @override
  State<VillageEmissionReportPage> createState() => _VillageEmissionReportPageState();
}

class _VillageEmissionReportPageState extends State<VillageEmissionReportPage> {
  ...
  Future<void> _loadReport() async {
    ...
    try {
      ...
      final uri = Uri.parse(
        'https://student.crru.ac.th/651463011/LowCarbonAPI/api/report/village-range-summary'
  ...
  // ===== helpers UI (ยกจากของเดิม) =====
  BarChartGroupData _bar(int x, double y) => BarChartGroupData(
        x: x,
        barRods: [
          BarChartRodData(
              toY: y, width: 22, borderRadius: BorderRadius.circular(6)),
        ],
      );

  Widget _co2BarChart({
    required double co2,
    required double reducing,
    required double net,
  }) {
  ...
  Widget _dailyChart(List<dynamic> dailyRows) {
    ...
    final maxValue = list.fold<double>(0.0, (mx, it) {
      final a = _num(it, 'total_CO2');
      final b = _num(it, 'reducing');
      return [mx, a, b].reduce((x, y) => x > y ? x : y);
    });
    final rawMax = (maxValue == 0 ? 1.0 : maxValue * 1.2);
    final niceMaxY = _niceNum(rawMax, round: false);
                ...
                child: BarChart(
                  BarChartData(
                    ...
                    titlesData: FlTitlesData(
                      leftTitles: AxisTitles(
                        sideTitles: SideTitles(
                          ...
                          interval: yStep,
                          getTitlesWidget: (value, meta) {
                            if (value < 0 || value > niceMaxY + 1e-6) {
                              return const SizedBox.shrink();
                            }
                            return Text(_fmtY(value));
                          },
                        ),
                      ),
                      ...
                      bottomTitles: AxisTitles(
                        sideTitles: SideTitles(
                          ...
                          getTitlesWidget: (value, meta) {
                            ...
                            final text = _dateLabel(list[idx]);
                    ...
                    barGroups: list.asMap().entries.map((entry) {
                      ...
                      final co2 = _num(item, 'total_CO2');
                      final reducing = _num(item, 'reducing');
  ...
  // small constants reused by _dailyChart
  static const double barWidth = 10;
  static const double labelBoxWidth = 58;
  ...
  Widget build(BuildContext context) {
    final Map<String, dynamic>? emissions = (data?['emissions'] is Map)
        ? Map<String, dynamic>.from(data!['emissions'])
        : null;

    final double reducing = (data?['reducing_total'] is num)
        ? (data!['reducing_total'] as num).toDouble()
        : 0.0;

    final double net = (data?['net'] is Map && (data!['net']['net_co2'] is num))
        ? (data!['net']['net_co2'] as num).toDouble()
        : 0.0;
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          // ถ้าต้องใช้ CatePie ในอนาคต สามารถยก _categoryPieCard จากหน้าเดิมมาใส่ได้เช่นกัน
        ],
      ),
    );
  }
